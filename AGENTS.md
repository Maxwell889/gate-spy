...
- 通用 RTL 恢复：`plan_recovery` / `recovery_tool_guide` / `probe_target` / `validate_expr` / `fit_template` / `infer_native_expr` / `check_polynomial_lowbits` / `split_control_cases` / `list_candidates` / `combine_case_expr` / `infer_pysr_expr(force=True)` / `assemble_rtl` / `verify_rtl_candidate` 组成 plan -> probe -> explicit validation or light attempt -> failure summary -> revised plan -> assembly/CEC 流程。恢复后用 `analyze_rtl_cost` / `propose_rtl_rewrites` / `optimize_rtl_round` 做 cost-guided RTL 优化。
- Reasoning Graph + 本地经验 RAG：`read_file(record_ir=True)` 或 `start_recovery_run(...)` 会启动 graph-only 记录，`.gate_spy/runs/<run_id>/` 只保存 `graph.json`、`graph.jsonl`、`summary.json` 和可选 `final_candidate.v`；普通 tool log、完整 report text、text preview、raw events 不落盘。`record_recovery_graph_node` 记录 `problem/experiment/observation/relation/validation/experience`；`query_recovery_experience` 从 `data/recovery_kb/` 检索已 promotion 的解决路径经验和归纳出的 `template_family`，`promote_recovery_experience` 只把已验证 `relation -> validation` 链或显式负面问题写入知识库，并从验证链归纳参数化模板族而不是保存答案公式。
...
- MCP 暴露的小工具包括 `plan_recovery(...)`、`recovery_tool_guide(...)`、`analyze_words(...)`、`probe_target(...)`、`validate_expr(...)`、`fit_template(...)`、`infer_native_expr(...)`、`check_polynomial_lowbits(...)`、`split_control_cases(...)`、`influence_profile(...)`、`list_candidates(...)`、`combine_case_expr(...)`、`infer_pysr_expr(...)`、`assemble_rtl(...)`、`verify_rtl_candidate(...)`、`analyze_rtl_cost(...)`、`propose_rtl_rewrites(...)`、`optimize_rtl_round(...)`。`plan_recovery` 是 agent-facing 第一入口，会按 output support/cone/role 分组，给出 per-cluster plan steps，并可通过 `feedback` 修订计划；每个 step 会显示 effort、search space、risk、prerequisites 和 why_cheapest。
...
- 新增本地经验 RAG/知识库：`query_recovery_experience(...)` 使用确定性 token/fingerprint 检索，不依赖外部 embedding 或云向量库；`query_recovery_memory(...)` 保留为兼容外壳。RAG 命中只返回触发条件、建议小实验、关系归纳、归纳出的 `template_family`、验证来源和负面经验；检索结果不能直接进入 `C#`，必须再经 full-support `validate_expr` 或 CEC。
...
[x] 扩展模板和经验归纳：`fit_template` 增加 bounded offset subtract/comparator 族，覆盖 `X_ext - Y_ext +/- CONST` 与 `X_ext CMP (Y_ext +/- CONST)`；`promote_recovery_experience` 会从已验证 `relation -> validation` 链归纳 `template_family` 并进入本地经验 RAG，后续查询返回模板族和建议实验，不返回可直接接受的答案公式。
[x] 增加 cost-guided RTL rewrite v1：新增 `analyze_rtl_cost`、`propose_rtl_rewrites`、`optimize_rtl_round`，覆盖 exact CSE、重复 sign/zero extension hoisting、mux-plus-common-add factoring、initializer concat-to-affine/conditional-subtract/narrow modular-subtract、signed-alias collapse、output-relation predicate factoring；优化轮使用 cost 降低 + 短 CEC 筛选，最终仍需长 CEC。
